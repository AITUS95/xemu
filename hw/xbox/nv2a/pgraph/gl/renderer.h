                                                     ...
                                                     bool enabled)
{
    ...
    if (enabled) {
        r->gl_vertex_attrib_value_valid[index] = false;
...
static inline bool pgraph_gl_set_vertex_attrib_value(PGRAPHGLState *r,
                                                     unsigned int index,
                                                     const float value[4])
{
    if (r->gl_vertex_attrib_value_valid[index] &&
        !memcmp(r->gl_vertex_attrib_value[index], value,
                4 * sizeof(float))) {
        return false;
    }
