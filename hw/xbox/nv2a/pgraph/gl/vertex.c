...
void pgraph_gl_bind_vertex_attributes_from_vram_snapshot(
    NV2AState *d, unsigned int min_element, unsigned int max_element,
    unsigned int provoking_element)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
    ...
    for (int i = 0; i < NV2A_VERTEXSHADER_ATTRIBUTES; i++) {
        ...
        NV2A_DPRINTF("snapshot vertex data array format=%d, count=%d, "
                     "stride=%d\n", attr->format, attr->count, attr->stride);
        ...
        hwaddr attrib_data_addr = attr_data + attr->offset - d->vram_ptr;
        ...
        size_t copied_size = (size_t)(max_element + 1) * stride;
        assert(attrib_data_addr + copied_size <= memory_region_size(d->vram));

        glBindBuffer(GL_ARRAY_BUFFER, r->gl_inline_buffer[i]);
        glBufferData(GL_ARRAY_BUFFER, copied_size,
                     d->vram_ptr + attrib_data_addr, GL_STREAM_DRAW);

        if (needs_conversion) {
            glVertexAttribIPointer(i, gl_count, gl_type, stride, 0);
        } else {
            glVertexAttribPointer(i, gl_count, gl_type, gl_normalize, stride,
                                  0);
        }
